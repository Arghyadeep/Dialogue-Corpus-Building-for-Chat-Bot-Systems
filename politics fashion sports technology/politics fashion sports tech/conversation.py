...
datacomp = pd.read_csv('database.csv')
...
for j in range(len(data[0])*5):
    ...
    for i in range(len(data[1])):
        if list(data[1]['Input.slot1'])[i] == temp:
            ...
            conv_id[list(datacomp['Texts']).index((data[1]['Answer.more_detail'][i]))]=conv_id[j]
            conv_id[list(datacomp['Texts']).index((data[1]['Answer.sympathetic'][i]))]=conv_id[j]
...
for i in range(len(data[1])):
    ...
    comp_arr1.append(data[1]['Answer.more_detail'][i])
    comp_arr1.append(data[1]['Answer.sympathetic'][i])
...
for i in range(len(comp_arr1)):
    ...
    for j in range(len(data[2])):
        if list(data[2]['Input.slot2'])[j] == temp:
            ...
            conv_id[list(datacomp['Texts']).index((data[2]['Answer.more_detail'][j]))]=conv_id[i]
            conv_id[list(datacomp['Texts']).index((data[2]['Answer.sympathetic'][j]))]=conv_id[i]
